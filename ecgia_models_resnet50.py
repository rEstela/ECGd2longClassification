from tensorflow.keras.layers import (
    Input, Conv2D, MaxPooling2D, AveragePooling2D, GlobalAveragePooling2D, Dropout, BatchNormalization, Activation, Add, Flatten, Dense, GlobalAveragePooling1D, Reshape, Permute, Multiply)
...
def resnet50(input_shape=(501, 936, 1), n_classes = 1):

    X_input = Input(input_shape)

    X = X_input

    X = Conv2D(64, (9, 9), strides=(2, 2), name='conv1', kernel_initializer='glorot_uniform')(X)
    X = BatchNormalization(axis=3, name='bn_conv1')(X)
    X = Activation('relu')(X)
    X = MaxPooling2D((3, 3), strides=(2, 2))(X)
    X = Conv2D(64, (7, 7), strides=(2, 2), name='conv1x', kernel_initializer='glorot_uniform')(X)
    ...
    headModel = Dense(n_classes, activation='sigmoid', name='fc3',kernel_initializer='glorot_uniform')(headModel)

    model = Model(inputs=X_input, outputs=headModel, name='resnet50')

    return model

def seresnet50(input_shape=(501, 936, 1), n_classes = 1):
    """
    https://gist.github.com/hollance/8d30bf5c1622036d16c4f27bd0ec88bf - SE-RESNET50
    """
    ...
    X = convolutional_block(X, f=3, filters=[64, 64, 256], stage=2, block='a', s=1)
    X = identity_block(X, 3, [64, 64, 256], stage=2, block='b', se = True)
    X = identity_block(X, 3, [64, 64, 256], stage=2, block='c', se = True)


    X = convolutional_block(X, f=3, filters=[128, 128, 512], stage=3, block='a', s=2)
    X = identity_block(X, 3, [128, 128, 512], stage=3, block='b', se = True)
    X = identity_block(X, 3, [128, 128, 512], stage=3, block='c', se = True)
    X = identity_block(X, 3, [128, 128, 512], stage=3, block='d', se = True)

    X = convolutional_block(X, f=3, filters=[256, 256, 1024], stage=4, block='a', s=2)
    X = identity_block(X, 3, [256, 256, 1024], stage=4, block='b', se = True)
    X = identity_block(X, 3, [256, 256, 1024], stage=4, block='c', se = True)
    X = identity_block(X, 3, [256, 256, 1024], stage=4, block='d', se = True)
    X = identity_block(X, 3, [256, 256, 1024], stage=4, block='e', se = True)
    X = identity_block(X, 3, [256, 256, 1024], stage=4, block='f', se = True)

    X = X = convolutional_block(X, f=3, filters=[512, 512, 2048], stage=5, block='a', s=2)
    X = identity_block(X, 3, [512, 512, 2048], stage=5, block='b', se = True)
...
if __name__ == '__main__':
    base_model = resnet50(input_shape=(501, 936, 1), n_classes = 1)
    pass